	...
	<div class="container mx-auto max-w-4xl p-6">
		<div class="mb-6">
			...
			>
				...
				<span class="ml-1">Back to Profile</span>
			</button>
		</div>
		...
		{:else if group}
			...
			<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
				<h2 class="mb-4 border-b border-gray-200 pb-2 text-xl font-semibold">Members</h2>

				<ul class="divide-y divide-gray-200">
					{#each group.members.filter((m) => m.invitationStatus === 'accepted') as member}
						<li class="flex items-center justify-between py-3">
							<div class="flex items-center">
								<div class="h-10 w-10">
									{#if typeof member.user !== 'string' && member.user.profileImage && member.user.profileImage.length > 2}
										<img
								...
								<div class="ml-3">
									...
									<p class="text-sm text-gray-500">{getMemberEmail(member)}</p>
								</div>
							</div>
							<div class="flex items-center">
								<span
									class="rounded-full px-2 py-1 text-xs font-medium {getRoleBadgeClass(
										member.role
									)}"
								>
									{member.role}
				...
				{#if group.members.some((m) => m.invitationStatus === 'pending')}
					<div class="mt-6">
						<h3 class="mb-2 text-lg font-medium">Pending Invitations</h3>
						<ul class="divide-y divide-gray-200">
							{#each group.members.filter((m) => m.invitationStatus === 'pending') as member}
								<li class="flex items-center justify-between py-3">
									<div class="flex items-center">
										<div class="h-8 w-8">
											<div
												class="flex h-8 w-8 items-center justify-center rounded-full bg-gray-100 text-sm font-semibold text-gray-400"
											>
												?
											</div>
										</div>
